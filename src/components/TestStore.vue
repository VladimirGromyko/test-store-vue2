...
export default {
  ...
  watch: {
    /**
     * Функция rate - отслеживает изменения курса валюты и вызывает различные действия:
     * вызов функции setPrevGoods - запоминания предыдущего значения товарных остатков и цен товаров,
     * вызов функции updateCost - изменения цен товаров,
     * вызов функции handleChange - изменения данных по ценам у текущей выбранной товарной группы,
     * вызов функции updateBasket -для обновления цен товаров в корзине в зависимости от изменившегося курса валюты.
     ...
     * вызов функции updateCost - изменения цен товаров,
     * вызов функции handleChange - изменения данных по ценам у текущей выбранной товарной группы,
     * вызов функции updateBasket - для обновления цен каждой товарной позиции в корзине в зависимости от изменившихся
     * валютных цен и курса валюты.
     ...
     * в store и содержащую контрольную информацию о движении покупок в/из корзину(ы) и вызывает различные действия:
     * вызов функции purchasesControl - контроля за остатками товара на складе и в корзине
     * вызов функции handleChange - изменения данных по ценам у текущей выбранной товарной группы,
     */
    changePurchases(){
  ...
  methods: {
     ...
     * и вызывает функцию контроля остатков
     */
    setGoods() {
      this.setPrevGoods()
     ...
     * @returns {string} - возвращает наличие товаров (full) или отсутствие товаров (empty) в товарной группе
    ...
    addPurchase(item) {
      ...
      // Изменяем состояния покупок для дальнейшего контроля за остатками товара
    ...
    updateBasket (rate, id, priceCurrency) {
      ...
      if (purchases.length) {
        purchases.forEach((el) => {
          if (id && el.productId === id) {
            el.priceCurrency = priceCurrency
          }
          if (rate && el.rate !== rate) {
            el.rate = rate
          }
          const newPrice = +(el.priceCurrency * el.rate).toFixed(1)
          el.priceChange = newPrice < el.price ? 'success' : 'danger'
          el.price = newPrice
      ...
      this.$store.dispatch("updateBasket", {purchases, totalCost})
    },
